...
(function () {
  ...
  var WA_MSG = "Olá! Vim pelo site da Bellus e gostaria de confirmar a disponibilidade da data da minha festa. Vou preencher o formulário para receber a proposta.";
  ...
  function waFallbackLink(data) {
    var parts = [
      ...
      "Nome: " + (data.nome || ""),
      data.nomeParceiro ? "Aniversariante: " + data.nomeParceiro : "",
  ...
  if (form) {
    form.addEventListener("submit", function (ev) {
      ...
      var data = {
        ...
        mensagem: "[Aniversário] " + (fd.get("mensagem") || "").toString().trim(),
      };
      ...
      if (!/^\d{4}-\d{2}-\d{2}$/.test(data.dataCasamento)) return setFeedback("Informe a data da festa.", "error");
      if (data.dataCasamento < HOJE_ISO) return setFeedback("A data da festa precisa ser hoje ou no futuro.", "error");
