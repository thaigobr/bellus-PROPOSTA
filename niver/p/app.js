(function () {
  ...
  var FN_COBRANCA = "https://nngvxucybligmanbedrs.supabase.co/functions/v1/asaas-cobranca-niver";
  ...
  var ADDONS = [
    { id:"tempo-extra-filme", nome:"Tempo extra de filme", desc:"Mais minutos de edição no seu filme.", benef:"Seu filme com mais momentos e mais respiro narrativo.", kind:"quantity", unitPrice:990, downsellPrice:900, unitMinutes:5, maxUnits:2, hideForEntry:true },
    { id:"previa", nome:"Prévia", desc:"Um primeiro recorte do filme, em até 2 semanas após a festa.", benef:"A emoção mais recente, enquanto ainda está viva.", kind:"bonus" },
  ];
  var PAGAMENTOS = [
  ...
  function visibleAddons(){var p=selPkg();return ADDONS.filter(function(a){return !(a.hideForEntry&&p&&p.noTimeAddon);});}
  ...
  function setQty(id,q){
    ...
    // Downsell DESLIGADO: o preco promocional so existia aqui no front e o backend
    // sempre cobrou o valor cheio. Para reativar, o valor tem que vir do banco.
    P.qty[id]=q; invalidarCot(); paintConfig(); paintMbar(); cotarBreve();
  }
  ...
  function waFalar(){var pk=selPkg();var b=breakdown();return waBase("Olá, Bellus! Sobre a proposta de "+nomes(P.proposta)+" (aniversário em "+dataTxt()+"). Estou vendo a experiência "+pk.nome+" ("+brl(b.total)+") e gostaria de conversar.");}
  ...
  function waReservar(){var b=breakdown();var pk=selPkg();var pay=selPay();return waBase("Olá, Bellus! Quero reservar a minha data ("+nomes(P.proposta)+", "+dataTxt()+").\nExperiência: "+pk.nome+" ("+brl(b.total)+")\nCondição: "+(pay?pay.label:"-")+"\nComo seguimos?");}
  function waDataAlternativa(){return waBase("Olá, Bellus! Estou com a proposta de "+nomes(P.proposta)+" em mãos. Vi que a data "+dataTxt()+" já está reservada. Tenho muito interesse no trabalho de vocês e gostaria de verificar a disponibilidade para a minha data ou outras opções. Conseguem nos ajudar?");}
  ...
  // (asaas-cobranca-niver, modo cotacao). NAO existe formula de taxa aqui, de proposito:
  // numero errado e pior que numero nenhum. Sem cotacao, o cartao fica indisponivel.
  ...
  // O carrinho enviado usa SO os addons visiveis no pacote atual. Sem isso, um addon
  // escondido pela troca de pacote sumia da tela e continuava sendo cobrado.
  function qtyEnviado(){ var q={}; visibleAddons().forEach(function(a){ var n=P.qty[a.id]||0; if(n>0)q[a.id]=n; }); return q; }
  // Uma falha de rede nao pode deixar a pagina sem preco: tenta de novo antes de desistir.
  function cotar(tent){
    tent=tent||0; var seq=++cotSeq;
    return fetch(FN_COBRANCA,{method:"POST",headers:{"Content-Type":"application/json",apikey:ANON,Authorization:"Bearer "+ANON},body:JSON.stringify({slug:getSlug(),pkgId:P.pkgId,qty:qtyEnviado(),cotacao:true})})
  ...
  // Trocou pacote ou quantidade? A tabela antiga deixa de valer NA HORA (o mapa por
  // pacote sobrevive porque nao depende do carrinho). Melhor "calculando" que numero velho.
  function invalidarCot(){ P.cot = P.cot ? { cotacao:true, pacotes:P.cot.pacotes, deslocamento:P.cot.deslocamento } : null; }
  function cotarBreve(){ if(cotTimer)clearTimeout(cotTimer); cotTimer=setTimeout(cotar,250); }
  ...
  function abrirPix(cond, met){
    ...
    if(ehCartao){
      var tab=(ehSaldo?(P.cot&&P.cot.cartaoSaldo):(P.cot&&P.cot.cartao))||[];
  ...
  function paintExp(){
    ...
    var html=PACOTES.map(function(pk){var sel=pk.id===P.pkgId;var _p12=cotPkg(pk.id,12);var is12=_p12?brlC(_p12.parcela):"…";var sinal=Math.round(preco(pk)*0.2);
      return '<div class="pcard'+(sel?" sel":"")+'">'+(pk.id===rec?'<span class="pcard__rec">★ Recomendada</span>':'')+
        ...
        '<div class="pcard__price"><div class="pcard__parc"><span class="x">12x de</span><span class="v serif tnum">'+is12+'</span></div><div class="pcard__full">ou '+brl(preco(pk))+' no total</div><div class="pcard__sinal">Reserva a data com <b>'+brl(sinal)+'</b> <span>(sinal de 20%)</span></div></div>'+
  ...
  function summaryHtml(){
    ...
    var h='<div class="summary"><div style="display:flex;justify-content:space-between;align-items:baseline"><h3>Sua contratação</h3><span class="ev">'+esc(p.evento_tipo||"Aniversário")+(p.evento_data?" · "+dataCurta(p.evento_data):"")+'</span></div>';
  ...
  function waReservada(){
    ...
    return waBase("Olá, Bellus! Sou da proposta de "+nomes(P.proposta)+". Está tudo pago e estou ansioso pela festa ("+dataTxt()+")!");
  }
  ...
  function reservaPainelHtml(){
    ...
    var countTxt=dd==null?"":(dd>1?'Faltam <b>'+dd+' dias</b> para o grande dia':dd===1?'É <b>amanhã</b>! Falta só 1 dia':dd===0?'<b>É hoje!</b> Que a sua festa seja inesquecível':'Festa registrada · obrigado por confiar na Bellus');
    ...
    return '<section class="section" id="contratacao"><div class="container">'+
      ...
      '<div class="reserva">'+
        '<div class="rsv-hero">'+
          ...
          '<p class="rsv-msg">A sua data está <b>garantida</b>. A partir de agora, cada detalhe importa para nós: vamos cuidar de tudo para transformar a sua festa em um filme para reviver pelo resto da vida.</p>'+
        '</div>'+
  ...
  function setupReveal(){
    ...
    // Hero: vídeo boomerang (ida e volta) em loop, sensação de piscar o olho, sem travar a página.
  // v.load() é necessário porque o <video> é inserido via innerHTML.
  ...
  // Grão de filme (chiado) só no hero: desktop, 8fps, tiles pré-gerados.
  var grainDone=false;
  function grainFx(){
  ...
  function build(){
    ...
    var rows=[["Evento",p.evento_tipo||"Aniversário"],["Data",dataSemana(p.evento_data)],["Local",p.evento_local||"A definir"],["Cidade",p.evento_cidade||"A definir"]];
    ...
    var dataBanner = ocupada ? '<div class="datawarn"><p class="dw-t">Essa data já está reservada</p><p class="dw-d">A data '+esc(dataCurta(p.evento_data))+' já consta como reservada na nossa agenda. Mas adoraríamos registrar a sua festa. Se tiver um pouco de flexibilidade na data, fale com a gente que verificamos as opções na hora.</p><a class="btn btn-wa" href="'+waDataAlternativa()+'" target="_blank" rel="noopener">'+WA+' Falar sobre datas</a></div>' : '';
    ...
    if(statusFechado && ddEvt!=null){
      ...
      if(av[0]==="available"){
        ...
        var sn=hh%4;
        if(sn>0) sc='<p class="scarcity"><span class="scarcity-dot" aria-hidden="true"></span>'+sn+(sn===1?' pessoa está':' pessoas estão')+' buscando esta data</p>';
      }
      topInfo='<span class="avail '+av[0]+'"><span class="dot '+av[0]+'"></span>'+av[1]+'</span>'+sc;
    }
    var imgsNds=(p.imagens||[]);
    var galeria = false ? ('<section class="section" id="previa-noiva"><div class="container"><div class="shead"><p class="eyebrow">Sua prévia Noiva dos Sonhos</p><h2 class="serif">Você, antes do grande dia</h2><p class="sub">As imagens que você criou na experiência Noiva dos Sonhos. Baixe as que quiser guardar.</p></div><div class="nds-grid" id="nds-grid">'+imgsNds.map(function(im){return '<figure class="nds-fig"><img class="nds-img" src="'+esc(im.url)+'" alt="Sua prévia de noiva" loading="lazy"/><a class="nds-dl" href="'+esc(im.url)+'?download" target="_blank" rel="noopener">Baixar</a></figure>';}).join("")+'</div><p class="nds-note">Por segurança, essas imagens ficam disponíveis por tempo limitado e depois são removidas. Baixe as suas para guardar.</p></div></section>') : '';
    ...
    // O dia de voces
    '<section class="section" id="seu-evento"><div class="container"><div class="shead"><p class="eyebrow">A sua festa</p><h2 class="serif">Os detalhes que já conhecemos</h2><p class="sub">Partimos do que você já nos contou, sem precisar repetir nada.</p></div>'+dataBanner+
    ...
    galeria+
    // manifesto
    '<section class="section section--tint"><div class="container narrow"><p class="eyebrow">Por que registrar</p><p class="serif" style="font-size:clamp(1.9rem,5vw,2.8rem);line-height:1.08;margin-top:1rem">O que você não viu só existe no filme.</p>'+
      '<blockquote class="manifq serif">O que você sente assistindo não é o mesmo que viveu. Agora você consegue ver tudo: os detalhes, as reações e as emoções que passaram despercebidas enquanto a festa acontecia.</blockquote>'+
      '<p class="manif-fear serif">A memória desbota. O filme não. De tudo que você contrata para a festa, é o único que não dá para refazer depois.</p></div></section>'+
    ...
    // como funciona
    '<section class="section" id="como-funciona"><div class="container"><div class="shead"><p class="eyebrow">Como funciona</p><h2 class="serif">Do contrato ao seu filme</h2><p class="sub">Um caminho simples e sem surpresas, do primeiro passo à entrega.</p></div>'+
    ...
    (rsv?'':'<section class="section" id="experiencias"><div class="container"><div class="shead"><p class="eyebrow">As experiências</p><h2 class="serif">Escolha como guardar a sua festa</h2><p class="sub">Toque numa experiência para ver os valores e o que cada uma inclui. O resumo se atualiza na hora.</p></div><div id="r-exp"></div><p class="exp-note">O making of da preparação está incluso em todas, se a aniversariante quiser fazer.</p></div></section>'+
    '<section class="section section--tint" id="comparar"><div class="container"><div class="shead"><p class="eyebrow">Lado a lado</p><h2 class="serif">Qual experiência combina mais com a sua festa?</h2><p class="sub">O essencial para comparar, sem termos técnicos.</p></div><div id="r-comp"></div></div></section>')+
    '<div id="r-config"></div>'+
    '<section class="section--dark finalcta section">'+part(0.7)+'<div class="hero__glow"></div><div class="container"><h2 class="serif">'+(rsv?'Estamos ansiosos pelo grande dia':'Vamos guardar a sua festa?')+'</h2><p>'+(rsv?'A sua data está reservada e cada detalhe já é importante para nós. Qualquer novidade, é só chamar.':'Qualquer dúvida sobre a proposta, é só chamar. Será uma alegria registrar a sua festa.')+'</p><a class="btn btn-wa" id="cta-final" href="'+(rsv?waReservada():waFalar())+'" target="_blank" rel="noopener">'+WA+' Falar com a Bellus</a></div></section>'+
  ...
  fetch(FN+"?slug="+encodeURIComponent(slug)+(/[?&]preview=1/.test(location.search)?"&preview=1":""),{headers:{apikey:ANON,Authorization:"Bearer "+ANON}})
    .then(function(r){return r.json().then(function(b){return {ok:r.ok,b:b};});})
    .then(function(r){if(r.ok&&r.b&&r.b.proposta){P.proposta=r.b.proposta;var rec=P.proposta.pacote_recomendado;P.pkgId=(rec&&PACOTES.some(function(p){return p.id===rec;}))?rec:"niver-diamante";build();cotar();mostrarRetornoPagamento();try{if(window.fbq){var _vb=breakdown();fbq("track","ViewContent",{value:_vb.total||0,currency:"BRL",content_name:(selPkg()||{}).nome||"Proposta"});}}catch(e){}}else erro(r.b&&r.b.error?r.b.error:"Proposta não encontrada.");})
